...
//                    $table->text('' . $name_ar . '')->nullable();
    ...
    {
        $check = DB::table('driver_profile_status')->where('position',$request->pos)->first();
        if (!isset($check->position))
        {
            return Redirect('show/columns')->withError( 'Please Specify the propper Position' );
        }
...
//                $table->renameColumn($prev_ar, $column_ar);
    ...
    {
        if (Auth::user()->status == 'admin') {
            $check = DB::table('customer_profile_status')->where('position', $request->pos)->first();
            if (!isset($check->position)) {
                return Redirect('show/customer/columns')->withError('Please Specify the propper Position');
            }
